const STORAGE_KEYS = {
  cardapio: "cardapioHB",
  historico: "historicoHB",
  numeroPedido: "numeroPedidoHB",
  dataPedido: "dataPedidoHB",
  adminLogado: "adminLogadoHB"
};

let cardapio = JSON.parse(localStorage.getItem(STORAGE_KEYS.cardapio)) || [
...
function gerarNumeroPedido() {
  ...
  const dataSalva = localStorage.getItem(STORAGE_KEYS.dataPedido);
  let numero = localStorage.getItem(STORAGE_KEYS.numeroPedido);

  if (dataSalva !== hoje) {
    numero = 1;
    localStorage.setItem(STORAGE_KEYS.dataPedido, hoje);
  } else {
    numero = numero ? parseInt(numero, 10) + 1 : 1;
  }

  localStorage.setItem(STORAGE_KEYS.numeroPedido, numero);
  return numero;
}
...
function excluirPedidoHistorico(indiceOriginal) {
  const logado = localStorage.getItem(STORAGE_KEYS.adminLogado);
  if (logado !== "true") {
    alert("Acesso negado.");
    return;
  }
...
function login() {
  ...
  if (user === "Hamburgada" && pass === "Schetini1245") {
    localStorage.setItem(STORAGE_KEYS.adminLogado, "true");
...
function logout() {
  localStorage.removeItem(STORAGE_KEYS.adminLogado);
  location.reload();
}
...
function zerarNumeracaoPedido() {
  ...
  localStorage.removeItem(STORAGE_KEYS.numeroPedido);
  localStorage.removeItem(STORAGE_KEYS.dataPedido);
...
function exportarBackup() {
  ...
  const dados = {
    historico: JSON.parse(localStorage.getItem(STORAGE_KEYS.historico)) || [],
    cardapio: JSON.parse(localStorage.getItem(STORAGE_KEYS.cardapio)) || [],
    numeroPedido: localStorage.getItem(STORAGE_KEYS.numeroPedido) || "0",
    dataPedido: localStorage.getItem(STORAGE_KEYS.dataPedido) || ""
  };
  ...
  leitor.onload = function(e) {
    try {
      ...
      if (dados.cardapio) {
        ...
        localStorage.setItem(STORAGE_KEYS.cardapio, JSON.stringify(dados.cardapio));
      }

      if (dados.numeroPedido) {
        localStorage.setItem(STORAGE_KEYS.numeroPedido, dados.numeroPedido);
      }

      if (dados.dataPedido) {
        localStorage.setItem(STORAGE_KEYS.dataPedido, dados.dataPedido);
      }
